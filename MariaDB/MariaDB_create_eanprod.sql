...
##################################################################
##this Stored Procedure will use the TrainMetroStation table
## use like: CALL sp_train_from_point(40.451585,-3.690375,2);
## if you need more results just change the LIMIT number or create a parameter for it
## MAXREC - how many to return (usefull when too close to a large regional to get the international also)
##################################################################
DROP PROCEDURE IF EXISTS sp_train_from_point;
DELIMITER $$
CREATE PROCEDURE sp_train_from_point(IN lat double,lon double, maxdist int)
BEGIN
...
FROM trainmetrostationcoordinateslist AS a 
WHERE 1=1
HAVING distance < maxdist
ORDER BY distance ASC;
# to use LIMIT you need to use a prepared statement to avoid errors
