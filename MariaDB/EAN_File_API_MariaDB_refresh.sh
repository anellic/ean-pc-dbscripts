#!/bin/bash
#########################################################################
## This is similar to EAN_MariaDB_refresh.sh but use new File API      ##
...
if [ -z "$CID" ]; then
    echo "CID environment variable is required."
fi

if [ -z "$SIGNATURE" ]; then
    echo "SIGNATURE environment variable is required."
fi

if [ -z "$APIKEY" ]; then
    echo "APIKEY environment variable is required."
fi

if [ -z "$CID" ] || [ -z "$SIGNATURE" ] || [ -z "$APIKEY" ]; then
    echo "Example usage:"
    echo "CID=xxx SIGNATURE=yyyyy APIKEY=zzzzz ./EAN_File_API_MariaDB_refresh.sh"
    exit 1;
fi
...
do
    ...
    curl -o $FILE.zip -L "http://api.ean.com/ean-services/rs/hotel/v3/files?\
    cid=$CID&sig=$SIGNATURE&apiKey=$APIKEY&filename=$FILE"
